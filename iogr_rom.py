...
def generate_rom(filename, rom_path, rng_seed, mode_str="Normal", goal="Dark Gaia", logic_mode="Completable", statues_reqstr="4", start_mode="South Cape", variant="None", enemizer="None", firebird=False):
    ...
    f.write(b"\xCB" + qt.encode("   Thanks to all the") + b"\xCB" + qt.encode("  amazing playtesters!"))
    f.write(b"\xC9\x78\xCE\xCB" + qt.encode("   Also, many thanks to") + b"\xCB" + qt.encode(" Raeven0 for ASM support.") + str_endpause)
    ...
    f.write(qt.encode("-Apokalysme -Bonzaibier") + b"\xCB")
    f.write(qt.encode("-Atlas      -BOWIEtheHERO") + b"\xC9\xB4\xCE")

    f.write(qt.encode("-bryon w    -DerTolleIgel") + b"\xCB")
    f.write(qt.encode("-Crazyhaze  -djtifaheart") + b"\xCB")
    f.write(qt.encode("-DoodSF     -Keypaladin") + b"\xCB")
    f.write(qt.encode("-Eppy37     -Lassic") + b"\xC9\xB4\xCE")

    f.write(qt.encode("-Le Hulk    -Plan") + b"\xCB")
    f.write(qt.encode("-manafreak  -Pozzum Senpai") + b"\xCB")
    f.write(qt.encode("-Mikan      -Raeven0") + b"\xCB")
    f.write(qt.encode("-Mr Freet   -roeya") + b"\xC9\xB4\xCE")
