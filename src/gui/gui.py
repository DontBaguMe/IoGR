...
def generate_ROM():
    ...
    try:
        ...
        settings = RandomizerData(seed_int, get_difficulty(), get_goal(), get_logic(), statues.get(), get_enemizer(), get_start_location(),
            firebird.get(), ohko.get(), red_jewel_madness.get(), glitches.get(), boss_shuffle.get(), open_mode.get(), get_sprite(), room_clear_items.get())
...
tkinter.Label(mainframe, text="Room Reward Items").grid(row=14, column=0, sticky=tkinter.W)
tkinter.Label(mainframe, text="Sprite").grid(row=15, column=0, sticky=tkinter.W)
...
room_clear_items = tkinter.IntVar(root)
room_clear_items.set(0)
...
room_clear_items_checkbox = tkinter.Checkbutton(mainframe, variable=room_clear_items, onvalue=1, offvalue=0).grid(row=14, column=1)
sprite_menu = tkinter.OptionMenu(mainframe, sprite, *sprite_choices).grid(row=15, column=1)
