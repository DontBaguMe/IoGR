...
from src.randomizer import constants
from src.randomizer import quintet_text
...
class World:
    ...
    def enemize(self, f, rom_offset=0):
        ...
        for map in constants.MAPS:
            ...
            if constants.MAPS[map][3]:
                addr = rom.find(constants.MAPS[map][3], int("d8000", 16) + rom_offset)
                if addr < 0 or addr > int("daffe", 16) + rom_offset:
                    print("ERROR: Couldn't find header for map ", map)
                else:
                    f.seek(addr + constants.MAPS[map][4])
                    f.write(constants.ENEMY_SETS[newset][0])

            # Randomize each enemy in map
            addr_start = constants.MAPS[map][5]
            addr_end = constants.MAPS[map][6]
                ...
                while not done:
                    ...
                    if addr < 0 or addr > int(addr_end, 16) + rom_offset:
                        ...
                        if self.settings.enemizer.value == Enemizer.BALANCED.value and enemy == 102:
                            f.write(b"\x47")
                        elif map != 27 and self.settings.enemizer.value != Enemizer.BALANCED.value:  # Moon Tribe cave enemies retain same template
